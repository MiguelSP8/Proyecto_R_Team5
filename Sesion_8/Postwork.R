...
head(df)
# 
df$nse5f <- factor(df$nse5f,labels=c("Bajo","Med. bajo","Medio","Med. alto","Alto"))
...
# de gasto sería la razón entre el gasto en alimentos no saludables vs saludables,
# lo cual nos habla más de la toma de decisión en cuanto al uso de los recursos 
# disponibles en cada hogarble rateNS=ln_alns/ln_als
...
"
#
# Por un lado, se desea analizar los patrones de gasto como función del nivel socioeconómico, ingresos extra y si el hogar presenta IA.
#
# También se quiere estimar la probabilidad de que un hogar presente seguridad alimentaria, en función de ciertos factores como
# los patrones de gastos, ingresos extra, etc.
...
#p1 + annotate(geom="text", x=3, y=30, label="Scatter plot",
#              color="red")
p1
#
# Zona
...
# constituída la muestra que estamos analizando. La distribución de hogares conforme
# al nivel socioeconómico es aproximadamente uniforme, aumenando ligeramente hacia
# los niveles socioeconómicos altos. Hay aproximadamente una razon de 2:1 hogares en
# zona urbana respecto a los hogares en zona rural. Aproximadamente 3 de cada 4 
# hogares no percibe ingresos extra. De la misma forma, cerca de 3 de cada
# 4 hogares tiene jefe de hogar hombre. Existe una proporción mayor al 2:1 en 
# los hogares que precentan insuficiencia alimentaria respecto alos que no. La 
# distribución del número de habitantes en el hogar es cercana a una normal, 
...
# refin, IA y nse5f  usando la función group_by() y calculamos los observables 
# de interés mediante la función summarise()
#
# nse5f
#
...
# en términos relativos con relación a su gasto en ALS. PRUEBA DE HIPÓTESIS PENDIENTE
# 2) Las medidas de tendencia central coinciden, en lo general, tanto para los ALS
# como para los ALNS, lo que nos habla de distribuciones cercanas a una gaussiana.
# 3) Para todos los NSE, se presenta sesgo a la izquierdo (s<0) en la distribución 
...
# Esto se puede corroborar en la gráfica p1, ariba generada
# 4) Para todos los NSE, se presenta sesgo a la derecha (s>0) en la distribución 
# del ln(ALNS) y una curtosis menor y aproximadamente 3 (mesocúrtica). Esto nos dice
# queel el patrón de gasto en ALNS de los hogares que pertenecen al mismo NSE es
# menos uniforme en comparación con el patron de gasto en ALS. Esto se puede 
# corroborar visualmente en la gráfica p2 arriba generada.
#
# Zona
#
df.vis<-df%>%group_by(area)%>%summarise(avg_als=mean(ln_als),
                                        med_als=median(ln_als),
                                        sd_als=sd(ln_als),
                                        ses_als=skewness(ln_als),
                                        cur_als=kurtosis(ln_als),
                                        ...
                                        sd_rate=sd(rateNS),
                                        n=n())
df.vis.ar<- data.frame(df.vis)
df.vis.ar
# 
# Observaciones:
# 1) EN general, las distribuciones del ln_ALS y ln_ALNS siguen el mismo patrón descrito para
# NSE5F. 
# 2) Los hogares de la zona urbana gastan más en ALNS, de forma absoluta y relativa, 
# que los hogares de la zona rural.
#
# refin
#
...
# 2) Por el contrario, en promedio, los hogares con ingresos extra gastan menos
...
# respecto al NSE y la zona geográfica.
#
# Sexo
#
df.vis<-df%>%group_by(sexojef)%>%summarise(avg_als=mean(ln_als),
                                           med_als=median(ln_als),
                                           sd_als=sd(ln_als),
                                           ses_als=skewness(ln_als),
                                           cur_als=kurtosis(ln_als),
                                           ...
                                           sd_rate=sd(rateNS),
                                           n=n())
df.vis.sex<- data.frame(df.vis)
df.vis.sex
#
# Observaciones:
# 1) En promedio, los hogares con jefe de familia hombre gastan más, absoluta y 
# relativamente, en  ALNS que los hogares con jefe de familia
# mujer.
...
# 2) La distribución del ln_ALS y ln_ALNS sigue los mismos patrones descritos 
# respecto al NSE, la zona geográfica, etc.
#
# numpeho
#
df.vis<-df%>%group_by(numpeho)%>%summarise(avg_als=mean(ln_als),
                                           med_als=median(ln_als),
                                           sd_als=sd(ln_als),
                                           ses_als=skewness(ln_als),
                                           cur_als=kurtosis(ln_als),
                                           ...
                                           sd_rate=sd(rateNS),
                                           n=n())
df.vis.nu<- data.frame(df.vis)
df.vis.nu
#df.vis.nu%>%summarise(avg=sum(numpeho*n)/sum(n))
#
# edad jefe
#
df.vis<-df%>%group_by(edadjef)%>%summarise(avg_als=mean(ln_als),
                                           med_als=median(ln_als),
                                           sd_als=sd(ln_als),
                                           ses_als=skewness(ln_als),
                                           cur_als=kurtosis(ln_als),
                                           ...
                                           sd_rate=sd(rateNS),
                                           n=n())
df.vis.ed<- data.frame(df.vis)
df.vis.ed
#
# años edu
#
df.vis<-df%>%group_by(añosedu)%>%summarise(avg_als=mean(ln_als),
                                           med_als=median(ln_als),
                                           sd_als=sd(ln_als),
                                           ses_als=skewness(ln_als),
                                           cur_als=kurtosis(ln_als),
                                           ...
                                           sd_rate=sd(rateNS),
                                           n=n())
df.vis.edu<- data.frame(df.vis)
df.vis.edu
#
#
# Una vez analisados los patrones de gasto en ALS y ALNS como función de cada una
# de las variables de manera individual, ahora realizaremos una análisis comparativo
# de los patrones de consumo para cada una de las 20 combinaciones que resultan 
# de considerar en conjunto las tres variables de interés:
# IA (2 opciones) x refin (2 opciones) x NSE5F (5 opciones)= 20 combinaciones
#
# Primero, vamos a investigar un poco respecto a la distribución de datos. 
# Para ello, se calcularán algunas medidas de tendencia central y dispersión como
# lo son el promedio, la mediana, la desviación estándar, el sesgo y curtosis para
# el ln_ALS y el ln_ALNS, para los hogares pertenecientes a cada categoria.
#
#
# Seleccionamos datos de interes usando select(), agrupamos conforme a las variables 
# refin, IA y nse5f  usando la función group_by() y calculamos los observables 
# deseados mediante la función summarise(). Por último, ordenaremos los datos 
# en función del nse5f para fines de análisis
#
...
#
# En busca de tener mayor claridad respecto a la distribución de datos, podemos 
# recurrir su epresentación visual mediante la creacción de histogramas.
#
...
# argumento para establecer el color en una gráfica comparativa de las 
...
# categorias parece seguir una distribución normal. 
#
...
# enfocaremos en la tendencia (promedio del ln del gasto) del gasto en ALS, ALNS y el gasto relativo 
# rateNS. Con este fin, seleccionamos datos de interes mediante la función select(),
# agrupamos conforme a las variables refin, IA y nse5f  usando la función 
# group_by() y calculamos los observables de interés mediante la función 
...
"
# Tal vez la parte del análisis de las distribuciones deba ir en esta sección
#
# Para el cálculo de las probabilidades, vamos a trabajar con la razón entre el gasto en ALNS y ALS
#
# 1) ¿cuál es la probabilidad de que los hogares que no reciben ingesos extra 
# destinen relativamente más recursos al consumo de ALNS que el promedio de 
#los hogares que sí perciben inresos extra?
...
# en nuestro analisis previo, con base en la información de esta muestra, en relación 
# a la población.
#
# Recordando:
...
que los hogares que pertenecen a un NSE bajo?
  "
# Planteamiento de hipótesis:
#
# Hipótesis nula, Ho: mu(ln_alns)_NSE('alto') <=  mu(ln_alns)_NSE('bajo')
# Hipótesis alternativa, Ha: mu(ln_alns)_NSE('alto') >  mu(ln_alns)_NSE('bajo')
#
...
#
res <- t.test(x = df[df$nse5f == "Alto", "ln_alns"], y = df[df$nse5f == "Bajo", "ln_alns"],
       alternative = "greater",
...
# Planteamiento de hipótesis:
#
# Hipótesis nula, Ho: mu(rateNS)_NSE('alto') <=  mu(rateNS)_NSE('bajo')
# Hipótesis alternativa, Ha: mu(rateNS)_NSE('alto') >  mu(rateNS)_NSE('bajo')
#
# Se están comparando dos grupos, primero necesitamos analizar la varianza de las
# dos muestras (inferencia sobre la media de dos poblaciones)
#
...
# Hipótesis nula, Ho: ratio=var1/var2=1
# Hipótesis alternativa, Ha: ratio != 1
res <- var.test(df[df$nse5f == "Alto", "rateNS"],
                df[df$nse5f == "Bajo", "rateNS"],
                ratio = 1, alternative = "two.sided")
pv <- res$p.value[1]
# NC del 90%
if(pv < 0.1) {
...
"
6) ¿Existe evidencia estadística para concluir que, en promedio, la percepción de
ingresos extra y el hecho de padecer o no insuficiencia alimentaria en todos los
niveles socioeconómicos, tienen efectos sobre el la razón del ln del gasto en ALNS respecto
al gasto en ALS?
"
# Planteamiento de hipótesis:
# Ho: avg_rateNS_refin(si)_IA(si)_nse = avg_rateNS_refin(si)_IA(no)_nse = avg_rateNS_refin(no)_IA(si)_nse = avg_rateNS_refin(no)_IA(no)_nse
# Ha: Al menos uno es diferente.
#
# NSE Alto
#
...
# se cumple que pvalue < alpha
# Por tanto, a niveles de confianza estándar EEE para rechazar Ho en favor de la Ha
# Es decir, a niveles de confianza estándar EEE se que al menos uno de los 
# promedios es diferente. Esto implica que el nivel socioeconómico si tiene efecto
# sobre la razón entre el gasto en ALNS y ALS.
...
#
# Se quiere estimar la probabilidad de que un hogar presente seguridad alimentaria o no... 
...
# la probabilidad de padecer IA es 1.0326 veces la probabilidad sin dicho aumento
#
